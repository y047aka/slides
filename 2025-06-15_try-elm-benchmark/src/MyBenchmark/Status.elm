module MyBenchmark.Status exposing
    ( Status(..), progress
    , Error(..)
...
import MyBenchmark.Samples exposing (Samples)
...
progress status =
    case status of
        ...
        Pending _ samples ->
            toFloat (MyBenchmark.Samples.count samples) / toFloat (numBuckets * samplesPerBucket) |> clamp 0 1
